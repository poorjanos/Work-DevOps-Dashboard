...
p <- ggplot(t_fte_ticket_team, aes(x = MONTH_WORKTIMESHEET, y = FTE, group = APPGROUP)) +
  geom_line(aes(colour = APPGROUP)) +
  facet_wrap(~ TICKET, nrow = 2) +
  theme(
    axis.text.x = element_text(angle = 90),
...
### Normalized FTE (FTE/Ticket Count)
...
### Normalized FTE per Team (FTE/Ticket Count)
...
p <- ggplot(t_fte_ticket_team, aes(x = MONTH_WORKTIMESHEET, y = FTE_NORM, group = APPGROUP)) +
  geom_line(aes(colour = APPGROUP)) +
  facet_grid(.~TICKET) +
  theme(
    axis.text.x = element_text(angle = 90),
...
ggplotly(p, tooltip = c("x", "y", "colour")) %>%
  layout(
    legend = list(orientation = "v", y = 0.5, tracegroupgap = 1),
    margin = list(b = 100)
  )
```
